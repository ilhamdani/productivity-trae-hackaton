...
def get_storage_client(settings: Settings | None = None) -> Minio:
    ...
    return Minio(endpoint, access_key=s.s3_access_key, secret_key=s.s3_secret_key, secure=secure, region=s.s3_region)


def get_presign_client(settings: Settings | None = None) -> Minio:
    s = settings or get_settings()
    secure = s.public_s3_base_url.startswith("https://")
    endpoint = s.public_s3_base_url.replace("https://", "").replace("http://", "")
    return Minio(endpoint, access_key=s.s3_access_key, secret_key=s.s3_secret_key, secure=secure, region=s.s3_region)


def presign_put_object(*, object_name: str, content_type: str, settings: Settings | None = None) -> str:
    s = settings or get_settings()
    client = get_presign_client(s)
